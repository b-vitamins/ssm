...
dw_causal_conv_backward_cpu(const at::Tensor& grad_output,
  ...
  at::Tensor grad_out_conv =
      channels_first ? grad_output : grad_output.permute({0, 2, 1});
  grad_out_conv = to_compute(grad_out_conv, compute_dtype, device).contiguous();
  ...
  at::Tensor grad_pre_activation;
  if (act == "silu" || act == "swish") {
    auto sigmoid = 1 / ((-conv_out).exp() + 1);
    grad_pre_activation =
        grad_out_conv * (sigmoid * (1 + conv_out * (1 - sigmoid)));
  } else {
    grad_pre_activation = grad_out_conv;
  }

  c10::optional<at::Tensor> grad_bias = c10::nullopt;
  if (bias.has_value()) {
    auto grad_bias_compute = grad_pre_activation.sum({0, 2});
    grad_bias = grad_bias_compute.to(bias.value().scalar_type());
  }

  auto x_unfold = x_padded.unfold(2, kernel_size, 1);
  auto grad_weight_compute =
      (grad_pre_activation.unsqueeze(-1) * x_unfold).sum({0, 2});

  if (weight.dim() == 3) {
    grad_weight_compute = grad_weight_compute.unsqueeze(1);
  }

  auto grad_weight = grad_weight_compute.to(weight.scalar_type());

  auto grad_x_padded = at::conv_transpose1d(grad_pre_activation, weight_cast,
                                            c10::nullopt, {1}, {0}, {1},
                                            channels);
  at::Tensor grad_x_conv =
      pad > 0 ? grad_x_padded.narrow(2, pad, x_conv.size(2)) : grad_x_padded;
  at::Tensor grad_x = grad_x_conv;
  if (!channels_first) {
    grad_x = grad_x.permute({0, 2, 1}).contiguous();
  }
  grad_x = grad_x.to(x.scalar_type());
